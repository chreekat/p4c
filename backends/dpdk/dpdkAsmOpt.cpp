...
big_int EmitDpdkTableConfig::convertSimpleKeyExpressionToBigInt(const IR::Expression *k,
    ...
    if (k->is<IR::Constant>()) {
        ...
        ::P4::error(ErrorType::ERR_INVALID, "%1% invalid Member key expression", k);
        return -1;
    } else {
        ::P4::error(ErrorType::ERR_INVALID, "%1% invalid key expression", k);
        return -1;
    }
}
        ...
        for (auto args : *actionCall->arguments) {
            for (auto arg : args->expression->to<IR::ListExpression>()->components) {
                ...
                if (arg->is<IR::Constant>()) {
                    ...
                    ::P4::error(ErrorType::ERR_UNSUPPORTED, "%1% unsupported argument expression",
                                arg);
                    continue;
                }
            }
...
void EmitDpdkTableConfig::addRange(const IR::Expression *k, int keyWidth, P4::TypeMap *typeMap) {
    ...
    if (k->is<IR::DefaultExpression>()) {
        ...
        if (start > end)
            ::P4::error(ErrorType::ERR_INVALID, "%s Invalid range for table entry", kr->srcInfo);
        startStr = start;
        endStr = end;
    } else {
