...
class DiscoverStructure : public Inspector {
    ...
    void checkReserved(const IR::Node *node, cstring nodeName, cstring kind) const {
        ...
        if (it->second != kind)
            ::P4::error(ErrorType::ERR_INVALID, "%1%: invalid name; it can only be used for %2%",
                        node, it->second);
    }
 ...
 public:
    ...
    void postorder(const IR::Primitive *primitive) override {
        ...
        if (extrn) {
            ...
            if (ctr == nullptr) {
                ::P4::error(ErrorType::ERR_NOT_FOUND, "%1%: Cannot find counter", ctrref);
                return;
            }
            auto parent = findContext<IR::ActionFunction>();
            ...
            if (mtr == nullptr) {
                ::P4::error(ErrorType::ERR_NOT_FOUND, "%1%: Cannot find meter", mtrref);
                return;
            }
            auto parent = findContext<IR::ActionFunction>();
            ...
            if (reg == nullptr) {
                ::P4::error(ErrorType::ERR_NOT_FOUND, "%1%: Cannot find register", regref);
                return;
            }
            auto parent = findContext<IR::ActionFunction>();
            BUG_CHECK(parent != nullptr, "%1%: not within action", primitive);
...
class ComputeTableCallGraph : public Inspector {
 ...
 public:
    ...
    void postorder(const IR::Apply *apply) override {
        ...
        if (tbl == nullptr) {
            ::P4::error(ErrorType::ERR_NOT_FOUND, "%1%: Could not find table", apply->name);
            return;
        }
        auto parent = findContext<IR::V1Control>();
        if (!parent) {
            ::P4::error(ErrorType::ERR_UNEXPECTED, "%1%: Apply not within a control block?", apply);
            return;
        }
...
class FixExtracts final : public Transform {
    ...
    HeaderSplit *splitHeaderType(const IR::Type_Header *type) {
        ...
        auto fixed = ::P4::get(fixedPart, type->name.name);
    ...
    */
    class RewriteLength final : public Transform {
     ...
     public:
        explicit RewriteLength(const IR::Type_Header *header, const IR::Declaration *var)
            : header(header), var(var) {
            setName("RewriteLength");
        }
 ...
 public:
    ...
    const IR::Node *postorder(IR::MethodCallStatement *statement) override {
        ...
        auto ht = ::P4::get(structure->extractsSynthesized, mce);
        if (ht == nullptr)
            // not an extract
            return statement;
        ...
        auto assign =
            new IR::AssignmentStatement(mce->srcInfo, new IR::PathExpression(varName), lookahead);
                ...
                for (n++; n != range.second; n++) {
                    ...
                    if (e1->node_type_name() == e2->node_type_name()) {
                        if (e1->srcInfo.getStart().isValid())
                            ::P4::error(ErrorType::ERR_DUPLICATE, "%1%: same name as %2%", e1, e2);
                        else
...
class MoveIntrinsicMetadata : public Transform {
 ...
 public:
    explicit MoveIntrinsicMetadata(ProgramStructure *structure) : structure(structure) {
        CHECK_NULL(structure);
        setName("MoveIntrinsicMetadata");
    }
    const IR::Node *preorder(IR::P4Program *program) override {
        stdType = program->getDeclsByName(structure->v1model.standardMetadataType.name)
                      ->single()
                      ->to<IR::Type_Struct>();
        userType = program->getDeclsByName(structure->v1model.metadataType.name)
                       ->single()
                       ->to<IR::Type_Struct>();
        CHECK_NULL(stdType);
        CHECK_NULL(userType);
        ...
        if (intrField != nullptr) {
            ...
            if (nt == nullptr || !nt->is<IR::Type_Struct>()) {
                ::P4::error(ErrorType::ERR_INVALID, "%1%: expected a structure", tn);
                return program;
            }
    ...
    const IR::Node *postorder(IR::Member *member) override {
        ...
        if (pe->path->name == structure->v1model.parser.metadataParam.name) {
            ...
            return new IR::PathExpression(new IR::Path(
                member->expr->srcInfo,
                IR::ID(pe->path->name.srcInfo, structure->v1model.standardMetadata.name)));
        }
        return member;
    }
};
