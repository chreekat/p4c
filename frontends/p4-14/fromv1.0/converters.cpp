...
const IR::Node *ExpressionConverter::postorder(IR::Primitive *primitive) {
    if (primitive->name == "current") {
        ...
        if (!a->is<IR::Constant>() || !b->is<IR::Constant>()) {
            ::P4::error(ErrorType::ERR_INVALID, "%1%: must have constant arguments", primitive);
            return primitive;
        }
        auto aval = a->to<IR::Constant>()->asInt();
        auto bval = b->to<IR::Constant>()->asInt();
        if (aval < 0 || bval <= 0) {
            ::P4::error(ErrorType::ERR_INVALID, "%1%: negative offsets?", primitive);
            return primitive;
        }
...
const IR::Node *ExpressionConverter::postorder(IR::PathExpression *ref) {
    if (ref->path->name.name == "latest") {
        if (structure->latest == nullptr) {
            ::P4::error(ErrorType::ERR_INVALID, "%1%: latest not yet defined", ref);
            return ref;
        }
...
const IR::Node *StatementConverter::preorder(IR::Apply *apply) {
    ...
    if (apply->actions.size() == 0) {
        ...
        for (auto a : apply->actions) {
            if (a.first == "hit") {
                if (hit != nullptr)
                    ::P4::error(ErrorType::ERR_DUPLICATE, "%1%: Duplicate 'hit' label", hit);
                hit = a.second;
            } else if (a.first == "miss") {
                if (miss != nullptr)
                    ::P4::error(ErrorType::ERR_DUPLICATE, "%1%: Duplicate 'miss' label", hit);
                miss = a.second;
            } else {
                otherLabels = true;
            }
        ...
        if ((hit != nullptr || miss != nullptr) && otherLabels)
            ::P4::error(ErrorType::ERR_INVALID, "%1%: Cannot mix 'hit'/'miss' and other labels",
                        apply);
